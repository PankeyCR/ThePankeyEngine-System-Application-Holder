	...
	#if defined(pankey_Log) && (defined(TypeArrayMap_Log) || defined(pankey_Global_Log) || defined(pankey_HolderSystem_Log))
			...
			class TypeArrayMap : public ValueArrayMapCollection<K,V,H,M>{
				public:
					...
					virtual long getKeyType()const{
						...
						TypeArrayMapLog(pankey_Log_Statement, "getKeyType", Base::ClassCount<K>::get());
					...
					virtual long getValueType()const{
						...
						TypeArrayMapLog(pankey_Log_Statement, "getValueType", Base::ClassCount<V>::get());
